...
async function loadBanks() {
  try {
    const banks = await authFetch("/withdraw/banks");
    ...
    showToast("Unable to load banks");
  }
}
...
async function resolveAccount() {
  ...
  accountNameEl.textContent = "Verifying account...";

  try {

    const data = await authFetch(
      `/withdraw/resolve-account?account_number=${encodeURIComponent(accountNumber)}&bank_code=${encodeURIComponent(selectedBankCode)}`
    );
...
withdrawBtn?.addEventListener("click", async () => {
  ...
  if (accountNumber.length !== 10) {
    return showToast("Enter a valid 10-digit account number");
  }

  if (!verifiedAccountName) {
    return showToast("Please verify your bank account first");
  }
  ...
  try {
    ...
    const data = await authFetch("/withdraw", {
      method: "POST",
      body: JSON.stringify({
        amount,
        bankName,
    ...
    displayMessage(
      data.message || "Withdrawal submitted successfully",
      "success"
    );
...
(async function init(){
  ...
  try { await loadBanks(); } catch(e){ console.error(e); }
